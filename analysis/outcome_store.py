"""
outcome_store.py — the backtester's single accessor over the long-format
...
``score_performance`` columns; an incomplete horizon rename silently starved
consumers for months (config#1456 root cause). The root-cause fix makes the
horizon a PARAMETER: outcomes live in the long-format
``score_performance_outcomes`` table (one row per signal × horizon; DDL owned
by crucible-research ``archive/schema.py``, produced by alpha-engine-data
``collectors/signal_returns.py``), and consumers filter
...
discipline — consumers never reach around the artifact). Everything downstream
receives either the long frame (:func:`load_outcomes`) or a score_performance
DataFrame whose outcome columns have been re-sourced from the long store
(:func:`attach_outcomes`).

UNITS — the one deliberate compatibility conversion
---------------------------------------------------
The long store is CANONICAL and stores returns as DECIMALS (0.043). The legacy
wide columns store them as 2dp-rounded PERCENT (4.30) — a quirk of the wide
producer (``round(x * 100, 2)`` in alpha-engine-data Step 2). Downstream
consumers (veto missed-alpha reporting, accuracy tables, emails) were built on
the percent convention, so :func:`attach_outcomes` reproduces it EXACTLY
(same round-to-2dp) at this single documented boundary. Consumers that want
full-precision decimals read :func:`load_outcomes` directly. Do NOT add a
second decimal↔percent conversion anywhere else in this repo.
"""
...
# The long-format record fields (mirrors nousergon_lib.contracts outcome_record
# v1 + the producer's physical columns).
_LONG_COLUMNS = (
...
def load_outcomes(
    ...
    Args:
        db_path:  research.db path, or an already-open sqlite3 connection.
    ...
    rows (graceful-empty by design). A missing PRIMARY horizon in a store that
    otherwise has rows is a producer starvation bug — fail-loud via
    ...
    if unknown:
        raise ValueError(
            ...
            f"{policy.all_horizons} — a non-policy horizon read is exactly the "
            f"bug class config#1483 exists to kill"
        )
    ...
    try:
        if not store_exists(conn):
            # Pre-cutover / freshly-created DB: absent table is detectable, not
            # an exception — callers gate on emptiness exactly as they gated on
            # an unresolved wide column. WARN so a starved run is loud.
            logger.warning(
                "%s table absent from research.db — long-format store not yet "
                "populated (producer: alpha-engine-data signal_returns Step 2c)",
                _TABLE,
            )
...
def attach_outcomes(
    ...
    long-format store (the config#1528 physical cutover).

    For each policy horizon ``h``, the wide outcome columns named by
    ``policy.outcome_columns(h)`` (beat-SPY flag, stock/SPY returns, and — for
    the primary horizon — the canonical log-alpha) are DROPPED from ``df`` and
    replaced with values read from ``score_performance_outcomes``, joined by
    ``(symbol, score_date)``. Returns are converted decimal → 2dp percent to
    preserve the legacy wide-column units exactly (see module docstring);
    ``log_alpha`` stays decimal (it always was). Rows without a long-store
    record get NaN — the same "unresolved" signal consumers already gate on.
    ...
    long_df = load_outcomes(db_path, policy=policy)
    if not long_df.empty:
        long_df = long_df.copy()
        # score_performance loaders parse score_date to datetime; the store
        # keeps TEXT dates. Normalize the join key to match df's dtype.
    ...
    for h in policy.all_horizons:
        ...
        # (long-store field → wide column name, decimal→percent?) per horizon.
        mapping = [
        ...
        if h_rows.empty:
            for _, wide_name, _p in mapping:
                out[wide_name] = pd.NA
                out[wide_name] = out[wide_name].astype("float64")
            continue

        attach = h_rows[key + [f for f, _, _p in mapping]].rename(
