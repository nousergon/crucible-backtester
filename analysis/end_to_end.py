...
def _scanner_factor_counterfactual(conn, loadings: dict | None = None) -> dict:
    ...
    try:
        ...
        for d, g in m.groupby("eval_date"):
            ...
            # sector-neutral alpha within the cycle
            if g["sector"].notna().any():
                g["alpha_sn"] = g["alpha21"] - g.groupby("sector")["alpha21"].transform("mean")
            else:
                g["alpha_sn"] = g["alpha21"]

            # cross-sectional z-score each raw factor across this cycle's scanned
            # universe, then build sign-oriented sleeves + the equal-weight composite.
            ...
            # map scores back onto g (by ticker)
