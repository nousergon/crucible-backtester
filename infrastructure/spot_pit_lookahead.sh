#!/usr/bin/env bash
# infrastructure/spot_pit_lookahead.sh — PitParityLookahead stage: run the
# pit_parity LOOKAHEAD predictor pass on its own spot EC2 instance and
# publish its stats artifact to
# s3://{bucket}/parity/{run_date}/pit_stats_lookahead.json
...
# The sibling stages: spot_pit_walkforward.sh, spot_parity_replay.sh,
...
#
# Usage:
#   ./infrastructure/spot_pit_lookahead.sh
#   ./infrastructure/spot_pit_lookahead.sh --preflight-only
#   ./infrastructure/spot_pit_lookahead.sh --run-date 2026-08-08
#   ./infrastructure/spot_pit_lookahead.sh --help
...
SPOT_STAGE_NAME="pit-lookahead"
PIT_PASS="lookahead"
# SF state name this script asserts against (config-I7214) — this script IS
# the PitParityLookahead SF state, hardcoded 1:1, no flag-derived mapping
# needed.
_COVERAGE_STAGE="PitParityLookahead"

# Per-stage runtime budget (sf-pipeline-policy §4 — sized to THIS stage, not
# the old bundle): healthy lookahead pass ≈ 12.5-13 min measured
