...
#
# Sourced by `_spot_common.sh`, which every per-stage launcher here sources.
...
# rather than reading another repo at runtime, the same terms this repo's
# `_spot_common.sh` header already states. The durable fix, named in
...
# `krepis.ec2_spot launch` walks types x subnets IN ORDER, and c6i.large also
# leads THIS repo's default rotation (alpha-engine-config-I11412). So every
# relaunch, both the launcher's own `exec` and any SF re-issue of the same
# state, went straight back into the pool being reclaimed, and nothing ever
# escalated to on-demand. The backtester stages run later in the same weekly
# SF, so the same reclaim pattern fails them the same way.
#
