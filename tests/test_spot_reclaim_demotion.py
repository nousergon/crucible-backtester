...
This is the backtester port of nousergon-data's
...
``INSTANCE_TYPES`` from its head, and c6i.large is the head of this repo's
default rotation too (alpha-engine-config-I11412). Nothing ever escalated to
on-demand. Separately, every per-stage script here armed its cleanup trap on
the line AFTER ``spot_common_launch_instance``, so an ec2_spot capacity (64)
or spot-quota (65) refusal exited with no handler at all.

These tests drive the REAL launch path (``_spot_common.sh``'s
``spot_common_launch_instance`` and the ``cleanup`` EXIT trap, including the
...
#: Every stage script that launches through ``spot_common_launch_instance``.
_STAGE_SCRIPTS = (
    "spot_backtester.sh",
    "spot_evaluator.sh",
    "spot_parity.sh",
...
# A per-stage launcher shaped exactly like spot_parity.sh's control flow:
# source, init defaults, launch (spot_common_launch_instance arms the EXIT
# trap itself, I11573), run a workload that dies. REPO_ROOT is a scratch dir
# with no ops_alerts module, so cleanup's alert fan-out fails at import (its
# own `|| echo` path) and never reaches SNS.
_STAGE = """#!/usr/bin/env bash
set -euo pipefail
REPO_ROOT="{repo_root}"
SPOT_STAGE_NAME="${{SPOT_STAGE_NAME:-parity}}"
source "{infra}/_spot_common.sh"
spot_common_init_defaults
_ORIG_ARGS=("$@")
spot_common_launch_instance
echo "ATTEMPT $SPOT_ATTEMPT ran on $INSTANCE_ID"
...
class _Rig:
    def __init__(self, tmp_path: Path) -> None:
        ...
        self.stage.write_text(_STAGE.format(infra=_INFRA, repo_root=self.repo_root))
...
def test_default_budget_goes_on_demand_only_on_the_last_attempt(rig: _Rig) -> None:
    """This repo's default budget is MAX_SPOT_ATTEMPTS=4: three spot attempts,
    each off the pools already reclaimed, then on-demand."""
    proc = rig.run(rig.stage, RUN_TOKEN="exec-default")
    launches = rig.launches()
    assert [launch["on_demand"] for launch in launches] == [False, False, False, True], _why(proc)
    assert [launch["types"][0] for launch in launches[:3]] == [
        "c6i.large",
        "c6a.large",
        "c7i.large",
    ]
    assert launches[3]["types"][-3:] == ["c6i.large", "c6a.large", "c7i.large"]
    assert proc.returncode == 7, _why(proc)
...
def test_sf_reissue_inherits_the_demotion(rig: _Rig) -> None:
    ...
    assert "spot reclaim recorded: c6i.large@subnet-a61ec0fb" in first.stderr, _why(first)
...
def test_stage_capacity_exhausted_at_launch_relaunches_on_demand(rig: _Rig) -> None:
    """ec2_spot exit 64 inside ``spot_common_launch_instance`` used to exit the
    stage BEFORE its cleanup trap was installed: no classification, no
    relaunch, no on-demand rung. The launch now arms the trap itself."""
...
def test_stage_spot_quota_goes_on_demand_before_the_final_attempt(rig: _Rig) -> None:
    ...
    the very next attempt is on-demand even with budget left (the default
    budget here is 4)."""
    _refuse_first_launch(rig, 65)
    proc = rig.run(rig.stage, RUN_TOKEN="exec-quota-4", FAKE_WORKLOAD_RC="0")
    launches = rig.launches()
    assert len(launches) == 2, _why(proc)
    assert launches[1]["on_demand"] is True, _why(proc)
...
def test_stage_capacity_refusal_with_budget_left_retries_spot(rig: _Rig) -> None:
    ...
    _refuse_first_launch(rig, 64)
    proc = rig.run(rig.stage, RUN_TOKEN="exec-cap-4", FAKE_WORKLOAD_RC="0")
    launches = rig.launches()
    assert len(launches) == 2, _why(proc)
...
def test_launch_arms_the_exit_trap_before_launching() -> None:
    """Structural: the trap is armed INSIDE ``spot_common_launch_instance``,
    ahead of the krepis launch call, so no stage script can place it too
    late, and no stage script re-arms it after the launch."""
    text = _COMMON.read_text()
    body = text[text.index("spot_common_launch_instance() {") :]
    body = body[: body.index("\n# ── Staging teardown")]
    assert "spot_common_install_cleanup_trap" in body
    assert body.index("spot_common_install_cleanup_trap") < body.index(
        "-m krepis.ec2_spot launch"
    )
    for stage in _STAGE_SCRIPTS:
        stage_text = (_INFRA / stage).read_text()
        launch_at = stage_text.index("\nspot_common_launch_instance")
        assert "spot_common_install_cleanup_trap" not in stage_text[launch_at:], (
            f"{stage}: re-arming the trap after the launch is where I11574 hid"
        )
