...
After the 2026-05-12 ``.env`` → SSM migration (PR 5 of the arc), every
...
silently re-introduce an ``os.environ.get("GMAIL_APP_PASSWORD")`` style read.

Non-secret env vars (``LANGCHAIN_PROJECT``, ``EMAIL_SENDER``, ``S3_BUCKET``,
``USE_REAL_ARCTICDB``, etc.) are allowed for now — they migrate to
