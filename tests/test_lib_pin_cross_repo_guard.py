...
coordinated pin bump in BOTH merge orders (backtester-first and
predictor-first), and `test_removing_the_lookaside_breaks_both_orders` (run
...
class TestBothMergeOrders:
    ...
    guard's OWN/SIBLING are fixed to backtester/predictor, so "both orders"
    from this repo's side means: backtester's PR runs BEFORE predictor's
    (this repo is the first half, needs the I7934 lookaside) and backtester's
    PR runs AFTER predictor's already merged (this repo is the second half,
    sibling@main already matches -- plain equality, no lookaside needed).
    crucible-predictor's mirror of this file exercises the same two shapes
    from predictor's side, covering the "predictor first" order."""

    def test_this_repo_first_partner_pr_open_green(self) -> None:
        # backtester PR open with NEW pin; predictor main still OLD; predictor
        # carries an open partner PR already pinning NEW.
        v = evaluate(
            own_pin=NEW,
            sibling_main_pin=OLD,
            sibling_open_prs=[_pr(534, NEW)],
        )
        assert v.ok
        assert v.lines[0].startswith("::notice::co-install parity DEFERRED")
        assert "crucible-predictor#534" in v.lines[0]
        assert v.lines[1] == (
            "co-install parity OK (lockstep partner in flight): "
            "crucible-predictor#534 pins v0.124.77"
        )

    def test_this_repo_second_after_sibling_already_merged_new(self) -> None:
        # predictor's half already merged (sibling@main is now NEW); this
...
class TestRedPaths:
    def test_no_partner_pr_is_red(self) -> None:
        ...
        assert "NO open crucible-predictor PR pins" in v.lines[0]
...
def test_removing_the_lookaside_breaks_both_orders() -> None:
    ...
    assert evaluate(NEW, OLD, [_pr(534, NEW)]).ok  # true today, WITH the lookaside
