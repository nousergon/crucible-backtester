...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.26.0" in text, (
            ...
            "``(instance_type × subnet)`` on ``InsufficientInstanceCapacity`` "
            "/ ``InsufficientHostCapacity`` / ``Unsupported`` / "
            "``InvalidAvailabilityZone`` / ``SpotMaxPriceTooLow``. Direct "
            "fix for the 2026-05-22 dry-pass failure where THIS REPO's "
            "Evaluator invocation hit InsufficientInstanceCapacity for "
            "c5.large in us-east-1f after the trap-fix unblocked the "
            "path. ``infrastructure/spot_backtest.sh`` now invokes "
            "``$LIB_PYTHON -m alpha_engine_lib.ec2_spot launch ...`` with "
            "all 6 default-VPC subnets + ``c5.large,m5.large,c6i.large,"
            "c5a.large`` fallback list. Bumped from v0.25.0 2026-05-22 "
            "late evening (lib PR #58). Update this test if the pin "
