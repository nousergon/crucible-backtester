...
class TestLibVersionPin:
    ...
    def test_requirements_pins_lib_to_stable_tag(self):
        ...
        assert "@v0.33.0" in text, (
            "alpha-engine-lib should pin to v0.33.0 (the latest stable "
            "tag at 2026-05-25, lifting "
            "``alpha_engine_lib.cost.record_anthropic_call`` chokepoint "
            "so backtester can adopt the cost-telemetry primitive without "
            "re-mirroring it). Earlier versions retained per the changelog "
            "scan: v0.26 (``ec2_spot`` Python CLI for spot-launch capacity "
            "rotation, primary Saturday-SF infra resilience surface used "
            "by ``infrastructure/spot_backtest.sh`` via "
            "``$LIB_PYTHON -m alpha_engine_lib.ec2_spot launch ...``), "
            "v0.27 (NYSE-calendar dates freshness), v0.28 "
            "(``pipeline_status`` SF projection), v0.29 (secrets-redacting "
            "log filter), v0.30 (Anthropic SDK adapter + packaged default "
            "pricing), v0.31 (server-tool fee tracking), v0.32 "
            "(``eval_type_backport`` on py3.9 for Pydantic), v0.33 "
            "(``record_anthropic_call`` capture chokepoint). Update this "
