...
def analyze(research_db_path: str) -> dict:
    ...
    # gate enables sizing on rank-IC >= 0.05 with no significance test; this asks
    # whether that IC's bootstrap CI excludes zero. NEVER changes the
    # recommendation. Swallow rationale ("fail loud" carve-out): (a) failure =
    # observe instrumentation error on a SECONDARY path; the recommendation is
    ...
    if bool(_cfg.get("significance_observe_enabled", True)):
        try:
            ...
            significance_observe = observe_ic_gate(
                df["barrier_win_prob"], df["canonical_actual"],
                gate="barrier_sizing", cfg=_cfg,
            )
        except Exception as e:  # observe-only: must not break the optimizer
            logger.warning(
                "barrier_sizing significance_observe failed (non-fatal, observe-only): %s", e,
            )
...
def apply(result: dict, bucket: str, run_date: str | None = None) -> dict:
    ...
    if bool(_cfg.get("enforce_significance", False)):
        ...
        if significance_would_block(verdict):
            logger.info(
                "barrier_sizing: significance enforce BLOCKED promotion (config#1426)"
            )
            return {
                "applied": False,
                "reason": "barrier_sizing: blocked by significance enforce "
