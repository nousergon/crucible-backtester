...
Two subcommands:
  ...
  * ``single`` — replay one captured DecisionArtifact under a target
    model. Smoke testing + ad-hoc retrospective analysis.
  * ``batch``  — iterate over a date-range corpus × target models,
    emit per-(agent_id, target_model) concordance to CloudWatch, and
    persist a per-target-model batch summary. Production cadence runs
    this from the Saturday SF.
